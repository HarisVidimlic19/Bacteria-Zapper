...
const deleteBacteria = (bacteria, bool) => {
	...
	if (bool){
		score += Math.round(1 / bacteriaArr[index].r);
...
const collide = (bacteria) => {
	const bacteriaIndex = bacteriaArr.indexOf(bacteria);
	for (let i = 0; i < bacteriaArr.length - 1; i++) {
		if (bacteriaIndex < bacteriaArr.indexOf(bacteriaArr[i])) {
			if (Math.sqrt(Math.pow((bacteria.x - bacteriaArr[i].x), 2) + (Math.pow((bacteria.y - bacteriaArr[i].y), 2))) <= bacteria.r + bacteriaArr[i].r) {
				console.log('collided!');
				merge(bacteria, bacteriaArr[i]);
				return;
			}
		}
	}
